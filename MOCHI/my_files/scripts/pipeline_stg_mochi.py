...
def evenly_spaced_guided_indices_with_offset(F: int, guided_ratio: float, offset: int, device) -> torch.LongTensor:
    guided_ratio = float(max(0.0, min(1.0, guided_ratio)))
    if F <= 0 or guided_ratio <= 0.0:
        return torch.empty((0,), device=device, dtype=torch.long)

    k = int(round(guided_ratio * F))
    k = max(1, min(F, k))

    step = max(1, int(round(F / k)))  # approximate spacing
    idx = (torch.arange(0, k, device=device) * step + offset) % F
    idx = torch.unique(idx).sort().values

    # keep endpoints for stability
    if idx.numel() == 0 or idx[0].item() != 0:
        idx = torch.cat([idx.new_tensor([0]), idx])
    if idx[-1].item() != F - 1:
        idx = torch.cat([idx, idx.new_tensor([F - 1])])

    return torch.unique(idx).sort().values
...
def evenly_spaced_guided_indices(F: int, guided_ratio: float, device: torch.device) -> torch.LongTensor:
    guided_ratio = float(guided_ratio)
    guided_ratio = max(0.0, min(1.0, guided_ratio))
    ...
    idx = torch.linspace(0, F - 1, steps=k, device=device).round().to(torch.long)
    idx = torch.unique(idx)

    # ensure endpoints for stability
    if idx.numel() == 0 or idx[0].item() != 0:
        idx = torch.cat([idx.new_tensor([0]), idx])
    if idx[-1].item() != F - 1:
        idx = torch.cat([idx, idx.new_tensor([F - 1])])
...
class MochiSTGPipeline(DiffusionPipeline, Mochi1LoraLoaderMixin):
    ...
    @property
    def do_motion_guidance(self):
        return self._motion_scale != 0.0
    ...
    def _apply_motion_guidance(self, noise_pred: torch.Tensor, motion_base: torch.Tensor) -> torch.Tensor:
        # noise_pred, motion_base: [B,C,F,H,W]
        if not self.do_motion_guidance:
            return noise_pred

        F_lat = motion_base.shape[2]
        start_f = max(1, int(self._motion_only_from_frame))
        if F_lat <= start_f:
            return noise_pred

        prev = motion_base[:, :, start_f - 1 : F_lat - 1, :, :]
        if self._motion_detach_prev:
            prev = prev.detach()

        curr = motion_base[:, :, start_f:, :, :]
        motion_term = (curr - prev)  # "change from last frame"

        # push *further* in that direction: pred_f += w*(pred_f - pred_{f-1})
        noise_pred[:, :, start_f:, :, :] = noise_pred[:, :, start_f:, :, :] + self._motion_scale * motion_term
        return noise_pred
    ...
    def __call__(
        ...
        # Guidance dropout
        guided_ratio: float = 1.0,   # 1.0 = normal CFG on all frames
        # Motion guidance
        ...
        self._motion_detach_prev = motion_detach_prev
        self._motion_only_from_frame = motion_only_from_frame
        ...
        use_sparse_uncond = (
            ...
            and (cfg_uncond_reference in ["nearest-short", "dropout-short"])
        )

        if self.do_classifier_free_guidance and not self.do_spatio_temporal_guidance and (not use_sparse_uncond):
            prompt_embeds = torch.cat([prompt_embeds_uncond, prompt_embeds_text], dim=0)
            prompt_attention_mask = torch.cat([prompt_mask_uncond, prompt_mask_text], dim=0)
        ...
        # (optional, but nice) keep prompt_embeds defined for callbacks/debug:
        if use_sparse_uncond:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # --- Guidance Dropout mask (structured, evenly spaced) ---
                # Build once per sample, in LATENT frame units.
                # --- Resample guidance-dropout mask every denoising step ---
                F_lat = latents.shape[2]
                offset = i % F_lat  # changes every step
                guided_idx = evenly_spaced_guided_indices_with_offset(F_lat, guided_ratio, offset, latents.device)

                frame_guidance_mask = torch.zeros((F_lat,), device=latents.device, dtype=torch.float32)
                frame_guidance_mask[guided_idx] = 1.0
                ...
                # timestep broadcast helper
                B = latents.shape[0]

                if use_sparse_uncond:
                    timestep_full = t.expand(B).to(latents.dtype)
                    ...
                    if cfg_uncond_reference == "nearest-short":
                        interval_lat = max(1, cfg_anchor_interval // self.vae_temporal_scale_factor)
                        anchors = torch.arange(0, F1, interval_lat, device=latents.device, dtype=torch.long)
                        if anchors[-1].item() != F1 - 1:
                            anchors = torch.cat([anchors, anchors.new_tensor([F1 - 1])])

                    elif cfg_uncond_reference == "dropout-short":
                        anchors = guided_idx  # <- you already sample this every step
                        if anchors.numel() == 0:
                            noise_pred = noise_pred_text
                            # motion guidance still ok
                            noise_pred = self._apply_motion_guidance(noise_pred, motion_base=noise_pred_text)
                            # skip to scheduler.step
                            # (just wrap this branch in an else to avoid duplicating scheduler code)
                            goto_scheduler = True
                        else:
                            goto_scheduler = False
                    else:
                        raise ValueError(cfg_uncond_reference)

                    if not (cfg_uncond_reference == "dropout-short" and anchors.numel() == 0):
                        latents_u = latents.index_select(dim=2, index=anchors)  # [B,C,A,H,W]
                        timestep_u = t.expand(B).to(latents.dtype)

                        noise_pred_uncond_small = self.transformer(
                            hidden_states=latents_u,
                        ...
                        noise_pred_text_a = noise_pred_text.index_select(dim=2, index=anchors)
                        guided_a = noise_pred_uncond_small + self.guidance_scale * (noise_pred_text_a - noise_pred_uncond_small)

                        noise_pred = noise_pred_text.clone()
                        noise_pred.index_copy_(dim=2, index=anchors, source=guided_a)
